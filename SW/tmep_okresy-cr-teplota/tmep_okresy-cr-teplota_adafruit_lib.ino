 ...
 * and show on the map. 
 * For Wi-Fi settings see config.h
 * 
 ...
 *
 * LaskaKit Interaktivní Mapa ČR WS2812B: https://www.laskakit.cz/laskakit-interaktivni-mapa-cr-ws2812b/
 */
    ...
    if ((millis() - lastTime) > timerDelay || first_time == 0) {
        if(WiFi.status()== WL_CONNECTED){
            ...
            for (int LED = 0; LED <= LEDS_COUNT - 1; LED++) 
            {
              ...
              strip.setPixelColor(LED, strip.ColorHSV(color * 256)); // Assuming Wheel function is generating HSV colors
            }
